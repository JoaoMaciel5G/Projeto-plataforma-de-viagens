...
export default function NavBar(){
  ...
  const navbarClasses = `fixed bg-zinc-50 w-full ${showNavbar && 'top-0'}`;
    ...
    function openSideBar(){
        stateSideBar ? setStateSideBar(!stateSideBar) : setStateSideBar(!stateSideBar)
    }
    function openInfos(){
        stateInfo ? setStateInfo(!stateInfo) : setStateInfo(!stateInfo)
    }

    return(
        <nav className={`${navbarClasses} flex justify-between shadow-[0_0_10px_0_rgba(0,0,0,0.75)] items-center justify-center py-[25px] px-2 z-10`}>
            {stateSideBar ? <ArrowLeft size={40} className="cursor-pointer lg:hidden" onClick={() => openSideBar()}/> : <Menu size={40} className="cursor-pointer lg:hidden" onClick={() => openSideBar()}/>}
            <div>
            ...
            <Info size={30} className="cursor-pointer lg:hidden" onClick={() => openInfos()}/>
